	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			ssrRenderList(__props.companiesSection.companies.data, (company) => {
				...
				}, "h-7 w-7 object-contain transition-transform duration-300 ease-in-out sm:h-9 sm:w-9 lg:h-10 lg:w-10"])}"${ssrRenderAttr("src", company.logo)} decoding="async" height="50" loading="lazy" width="50"><h2 class="text-center text-xs font-semibold leading-tight text-slate-900 sm:text-sm lg:text-base">${ssrInterpolate(company.name)}</h2></a>`);
			});
...
//# sourceMappingURL=Companies-okQubWFv.js.map