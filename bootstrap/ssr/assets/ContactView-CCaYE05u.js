...
import { n as _sfc_main$6, r as SpecularButton_default, t as useGetTitle } from "./useGetTitle-DejgjcG2.js";
...
function _sfc_ssrRender$1(_ctx, _push, _parent, _attrs) {
	_push(`<svg${ssrRenderAttrs(mergeProps({
		...
		viewBox: "0 0 24 24",
		xmlns: "http://www.w3.org/2000/svg"
	}, _attrs))}><path d="M16.5 12a4.5 4.5 0 1 1-9 0 4.5 4.5 0 0 1 9 0Zm0 0c0 1.657 1.007 3 2.25 3S21 13.657 21 12a9 9 0 1 0-2.636 6.364M16.5 12V8.25" stroke-linecap="round" stroke-linejoin="round"></path></svg>`);
}
...
_sfc_main$2.setup = (props, ctx) => {
	const ssrContext = useSSRContext();
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Components/Organisms/Contact/ContactContent.vue");
...
var _sfc_main$1 = {
	...
	setup(__props) {
		...
		return (_ctx, _push, _parent, _attrs) => {
			...
			if (unref(form).errors.message) _push(`<p class="mt-1 text-sm text-red-600">${ssrInterpolate(unref(form).errors.message)}</p>`);
			else _push(`<!---->`);
			_push(`</div><div><label class="mb-3 block text-start text-sm text-gray-700">${ssrInterpolate(fieldTranslations.value.services ?? "Services required")}</label><div class="grid grid-cols-1 gap-3 text-sm text-gray-700 sm:grid-cols-2"><!--[-->`);
			ssrRenderList(unref(form).services, (enabled, service) => {
				_push(`<label class="flex cursor-pointer items-center gap-2"><input${ssrIncludeBooleanAttr(Array.isArray(unref(form).services[service]) ? ssrLooseContain(unref(form).services[service], null) : unref(form).services[service]) ? " checked" : ""} class="h-4 w-4 rounded border-gray-300 text-primary-500 focus:ring-primary-400" type="checkbox"><span>${ssrInterpolate(service)}</span></label>`);
			});
			_push(`<!--]--></div>`);
			...
			_push(ssrRenderComponent(SpecularButton_default, {
				...
				default: withCtx((_, _push, _parent, _scopeId) => {
					if (_push) {
						if (!unref(form).processing) _push(`<span class="font-medium"${_scopeId}>${ssrInterpolate(__props.formContent.submit_label)}</span>`);
						else _push(`<span class="font-medium"${_scopeId}>${ssrInterpolate(__props.formContent.submitting_label)}</span>`);
					} else return [!unref(form).processing ? (openBlock(), createBlock("span", {
						key: 0,
						class: "font-medium"
					}, toDisplayString(__props.formContent.submit_label), 1)) : (openBlock(), createBlock("span", {
						key: 1,
						class: "font-medium"
					}, toDisplayString(__props.formContent.submitting_label), 1))];
				}),
				_: 1
			}, _parent));
...
_sfc_main$1.setup = (props, ctx) => {
	const ssrContext = useSSRContext();
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Components/Organisms/Contact/ContactForm.vue");
...
_sfc_main.setup = (props, ctx) => {
	const ssrContext = useSSRContext();
	(ssrContext.modules || (ssrContext.modules = /* @__PURE__ */ new Set())).add("resources/js/Pages/ContactView.vue");
	return _sfc_setup ? _sfc_setup(props, ctx) : void 0;
};
//#endregion
export { _sfc_main as default };

//# sourceMappingURL=ContactView-CCaYE05u.js.map