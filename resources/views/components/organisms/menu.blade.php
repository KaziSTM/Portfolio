...
<div>
    ...
    <div class="relative flex md:hidden">
        <div x-data="{ modalOpen: false }"
            ...
            {{-- Burger button --}}
            <div class="absolute z-[99] -top-6 end-1">
                <button @click="modalOpen=!modalOpen"
                        class="flex flex-col rotate-45 w-full px-5 py-4 font-semibold text-start select-none focus:border-none focus:ring-0">
                    ...
                    <div x-cloak x-show="!modalOpen"
                         :class="{ 'rotate-45': !modalOpen }"
                         class="relative flex w-4 h-4 duration-300 ease-out">
                        <div
                            class="absolute w-0.5 h-3/5 bottom-[14.5px] bg-neutral-900 group-hover:bg-neutral-800 rounded-full"></div>
                        <div :class="{ 'rotate-90': !modalOpen }"
                             class="absolute w-7/12 h-0.5 start-[1.5px] -top-[4.5px] ease duration-900 bg-neutral-900 group-hover:bg-neutral-800 rounded-full">
                        </div>
                    </div>
                </button>
            </div>
            ...
            <template x-teleport="body">
                <div x-show="modalOpen"
                     class="absolute top-0 start-0 z-[30] flex items-start justify-center w-screen h-screen"
                     x-cloak>
                            ...
                            <div class="flex flex-col pb-2">
                                @foreach ($navigation as $link)
                                    <a href="{{ route($link['route']) }}"
                                       wire:navigate
                                       class="relative flex cursor-default select-none hover:bg-primary-100 items-center rounded-sm px-2 py-1.5 text-sm outline-hidden transition-colors data-[disabled]:pointer-events-none data-[disabled]:opacity-50">
